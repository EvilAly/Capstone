...
function get_full_recipe($recName) {
    global $db;
    $query = "SELECT * FROM recipes 
        WHERE recName LIKE '%" . $recName . "%'";
    $statement = $db->prepare($query);
    $statement->execute();
    $recipes = $statement->fetchAll();
    $statement->closeCursor();
...
function get_recID ($recName) {
    global $db;
    $query = "SELECT recipeID FROM recipes"
            . "WHERE recName = :recName";
    $statement = $db->prepare($query);
    $statement->bindValue(':recName', $recName);
    $$statement->execute();
    $recipeID = $statement->fetchAll();
    $statement->closeCursor();
    return $recipeID;
}